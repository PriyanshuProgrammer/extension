import type { LoadScriptOptions } from '../common/load-script';
import type { PiOpts } from '../platform-implementation-js/platform-implementation';
declare global {
    var __test_origin: string | undefined;
}
export declare const LOADER_VERSION: string;
/**
 * Only works in the non-npm remote-loaded build for MV2 extensions.
 * @deprecated
 */
export declare function loadScript(url: string, opts?: LoadScriptOptions): Promise<void>;
export declare function load(version: number, opts?: Partial<PiOpts>): Promise<import("../platform-implementation-js/platform-implementation").PlatformImplementation>;
//# sourceMappingURL=inboxsdk.d.ts.map